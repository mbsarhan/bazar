...
const AddRealEstateForm = () => {
    ...
    const { country } = useLocation(); // Get the current country
    ...
    useEffect(() => {
        ...
        const src = videoFile instanceof File ? URL.createObjectURL(videoFile) : videoFile;
        const type = videoFile instanceof File ? videoFile.type || 'video/mp4' : videoFile.endsWith('.m3u8') ? 'application/x-mpegURL' : 'video/mp4';
        ...
        if (isEditMode) {
            const fetchAdData = async () => {
                try {
                    ...
                    if (adData?.imageUrls) setImages(adData.imageUrls);
                    if (adData?.videoUrl) setVideoFile(adData.videoUrl);
                } catch (err) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            images.forEach(image => {
                if (image instanceof File) dataToSubmit.append('images[]', image);
            });
            if (videoFile instanceof File) dataToSubmit.append('video', videoFile);
            removedMedia.forEach(filename => dataToSubmit.append('removed_media[]', filename));
    ...
    // 1. Determine the correct labels and list based on the country.
    const isSaudi = country.code === 'SA';
    const locationLabel = isSaudi ? 'المدينة *' : 'المحافظة *';
    const cityLabel = isSaudi ? 'الحي *' : 'المدينة / المنطقة *';
    const detailedAddressPlaceholder = isSaudi 
        ? "مثال: حي العليا، شارع الأمير محمد بن عبدالعزيز" 
        : "مثال: المزة - شارع الجلاء - بناء 22";
            ...
            <form onSubmit={handleSubmit}>
                ...
                <fieldset>
                    ...
                    <div className="form-group">
                        ...
                        <input type="text" id="title" name="title" value={formData.title} onChange={handleChange} placeholder="مثال: شقة فخمة بإطلالة رائعة في المالكي" className={errors.title ? 'input-error' : ''} />
                    </div>
                </fieldset>
                ...
                <fieldset>
                    ...
                    <div className="form-grid">
                        <div className="form-group"><label htmlFor="transaction_type">نوع الصفقة *</label><select name="transaction_type" value={formData.transaction_type} onChange={handleChange}>{transactionTypes.map(t => <option key={t} value={t}>{t}</option>)}</select></div>
                        <div className="form-group"><label htmlFor="realestate_type">نوع العقار *</label><select name="realestate_type" value={formData.realestate_type} onChange={handleChange}>{propertyTypes.map(p => <option key={p} value={p}>{p}</option>)}</select></div>
                    </div>
                </fieldset>
                    ...
                    <div className="form-grid">
                        <div className="form-group">
                            ...
                            <select name="governorate" value={formData.governorate} onChange={handleChange}>
                                {/* 3. Use the dynamic list */}
                                {locationsList.map(p => <option key={p} value={p}>{p}</option>)}
                            </select>
                        </div>
                        <div className="form-group">
                            {/* 4. Use the dynamic city/neighborhood label */}
                        ...
                        <div className="form-group full-width">
                            ...
                            {/* 5. Use the dynamic placeholder */}
                            <input type="text" name="detailed_address" value={formData.detailed_address} onChange={handleChange} placeholder={detailedAddressPlaceholder} className={errors.detailed_address ? 'input-error' : ''} />
                        </div>
                    </div>
                </fieldset>
                ...
                {/* ... other fieldsets ... */}
                 <fieldset>
                    ...
                    <div className="form-grid four-columns">
                        <div className="form-group"><label htmlFor="area">المساحة (م²) *</label><input type="text" name="area" value={formData.area} onChange={handleChange} placeholder='يقبل مساحة تقريبية' className={errors.area ? 'input-error' : ''} /></div>
                        <div className="form-group"><label htmlFor="bedroom_num">غرف النوم</label><input type="number" name="bedroom_num" value={formData.bedroom_num} onChange={handleChange} /></div>
                        <div className="form-group"><label htmlFor="bathroom_num">الحمامات</label><input type="number" name="bathroom_num" value={formData.bathroom_num} onChange={handleChange} /></div>
                        <div className="form-group"><label htmlFor="floor_num">رقم الطابق</label><input type="number" name="floor_num" value={formData.floor_num} onChange={handleChange} /></div>
                        <div className="form-group"><label htmlFor="building_status">حالة البناء *</label><select name="building_status" value={formData.building_status} onChange={handleChange}>{constructionStatuses.map(s => <option key={s} value={s}>{s}</option>)}</select></div>
                        <div className="form-group"><label htmlFor="cladding_condition">حالة الإكساء *</label><select name="cladding_condition" value={formData.cladding_condition} onChange={handleChange}>{finishingStatuses.map(f => <option key={f} value={f}>{f}</option>)}</select></div>
                    </div>
                </fieldset>
                ...
                <fieldset>
                    <legend>السعر</legend>
                    <div className="form-grid">
                        <div className="form-group"><label htmlFor="price">السعر المطلوب ({currencyLabel})</label><input type="text" name="price" value={formData.price} onChange={handleChange} /></div>
                        <div className="form-group checkbox-group price-checkbox"><input type="checkbox" id="negotiable_check" name="negotiable_check" checked={formData.negotiable_check} onChange={handleChange} /><label htmlFor="negotiable_check">السعر قابل للتفاوض</label></div>
                    </div>
                </fieldset>

                <fieldset>
                    <legend>التفاصيل والوسائط</legend>
                    <div className="form-group"><label htmlFor="description">وصف تفصيلي للعقار والميزات</label><textarea name="description" rows="5" value={formData.description} onChange={handleChange} placeholder="اكتب هنا عن ميزات العقار كالإطلالة، وجود مصعد، كراج، تدفئة..." className={errors.description ? 'input-error' : ''}></textarea></div>
                    <div className="form-group">
                                ...
                                {images.map((image, index) => (
                                    <div key={index} className="upload-box">
                                        <div className="image-preview">
                                            <img src={image instanceof File ? URL.createObjectURL(image) : image} alt={`extra ${index + 1}`} />
                        ...
                        <div className="video-uploader">
                            {videoFile ? (
                                <div className="video-preview">
                                    {isUploading ? (<ProgressBar progress={uploadProgress} />) : (videoOptions && <VideoPlayer options={videoOptions} />)}
                                ...
                                <div className="upload-box video-upload-box" onClick={() => videoInputRef.current.click()}>
                                    <div className="upload-placeholder"><UploadIcon /><span>اختر فيديو</span></div>
                                </div>
                            )}
                        </div>
                ...
                <button type="submit" className="submit-btn" disabled={isUploading || isSubmitting}>
                    {isSubmitting ? 'جاري الحفظ...' : (isUploading ? 'جاري رفع الفيديو...' : (isEditMode ? 'حفظ التعديلات' : 'نشر إعلان العقار'))}
                </button>
            </form>
        </div>
    );
